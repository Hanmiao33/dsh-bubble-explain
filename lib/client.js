		...
		*/
		function Dropdown(props) {
			...
			react.default.useEffect(() => {
				if (!open) return;
				const onDocDown = (ev) => {
					if (ref.current !== null && !ref.current.contains(ev.target)) setOpen(false);
				};
				const onKey = (ev) => {
			...
			}, react.default.createElement("span", { className: "bbl-dd-label" }, current?.label ?? props.value), react.default.createElement("span", { className: "bbl-dd-chevron" }, "▾")), open ? react.default.createElement("div", {
				className: "bbl-dd-menu",
				role: "listbox"
...
`;
    ...
    `;
		...
		function loadModels() {
			fetch(MODELS_ROUTE, { credentials: "same-origin" }).then((r) => r.ok ? r.json() : null).then((data) => {
				if (data === null) return;
		...
		function refreshSettings() {
			fetch(SETTINGS_ROUTE, { credentials: "same-origin" }).then((r) => r.ok ? r.json() : null).then((data) => {
				...
				modelState.effective = eff !== null && eff !== void 0 && typeof eff.provider === "string" && typeof eff.model === "string" ? {
		...
		function SettingsPage() {
			...
			const modelRows = [row("模型来源", react.default.createElement(Dropdown, {
				...
				onChange: (value) => {
					if (value === "") submitSettings({
						provider: "",
						model: ""
					});
					else submitSettings({
						provider: value,
						model: modelState.directory.models[value]?.[0]?.id ?? ""
					});
				}
			}))];
			if (!followDefault) modelRows.push(row("解释所用模型", react.default.createElement(Dropdown, {
				value: settings.model,
				options: providerModels.length === 0 ? [{
					value: settings.model,
					label: settings.model || "（该 provider 未声明模型）"
				}] : providerModels.map((entry) => ({
					value: entry.id,
					label: entry.name === entry.id ? entry.id : `${entry.name}（${entry.id}）`
				})),
				onChange: (value) => submitSettings({ model: value })
			})));
			const effectiveLine = modelState.effective === null ? "当前无法解析可用模型。" : `当前实际调用：${modelState.effective.provider} / ${modelState.effective.model}`;
			...
			})), row("思考强度", react.default.createElement(Dropdown, {
				value: settings.effort,
				options: EFFORT_OPTIONS.map((option) => ({
					value: option.value,
					label: option.label
				})),
				onChange: (value) => submitSettings({ effort: value })
			})), react.default.createElement("div", { className: "bbl-set-sub" }, "独立模型配置"), ...modelRows, react.default.createElement("p", { className: "bbl-set-note" }, `${effectiveLine}${modelState.error.length > 0 ? ` · ${modelState.error}` : ""}`));
		}
