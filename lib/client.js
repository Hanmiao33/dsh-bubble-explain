...
`;
    ...
    `;
		...
		function SettingsPage() {
			...
			const modelRows = [row("模型来源", react.default.createElement("select", {
				...
				key: "__default",
				value: ""
			}, "跟随对话默认模型"), providers.map((provider) => react.default.createElement("option", {
				key: provider.id,
				value: provider.id
			}, provider.name))))];
			if (!followDefault) modelRows.push(row("解释所用模型", react.default.createElement("select", {
				value: settings.model,
				onChange: (ev) => submitSettings({ model: ev.target.value })
			}, providerModels.length === 0 ? [react.default.createElement("option", {
				key: settings.model,
				value: settings.model
			}, settings.model || "（该 provider 未声明模型）")] : providerModels.map((entry) => react.default.createElement("option", {
				key: entry.id,
				value: entry.id
			...
			return react.default.createElement("div", { className: "bbl-settings" }, react.default.createElement("p", { style: {
				...
				key: option.value,
				value: option.value
			}, option.label)))), react.default.createElement("div", { className: "bbl-set-sub" }, "独立模型配置"), ...modelRows, react.default.createElement("p", { className: "bbl-set-note" }, `${effectiveLine}${modelState.error.length > 0 ? ` · ${modelState.error}` : ""}`));
		}
