...
import { DEFAULT_EFFORT, DEFAULT_MAX_CHARS, MAX_DEPTH, buildSystemPrompt, buildUserMessage, isUsableRoute, liveProviderIds, normalizeEffort, parseExplainRequest, resolveEffortForRoute, resolveModelRoute, } from './explain.js';
            ...
            void (async () => {
                ...
                if (method === 'POST' || method === 'PUT') {
                    ...
                    if (typeof body?.maxDepth === 'number') {
                        next.maxDepth = clampInt(body.maxDepth, 1, MAX_DEPTH, DEFAULT_SETTINGS.maxDepth);
                    }
                    if (typeof body?.maxChars === 'number') {
                        next.maxChars = clampInt(body.maxChars, 50, 1000, DEFAULT_SETTINGS.maxChars);
                    }
                    if (typeof body?.effort === 'string') {
                        next.effort = normalizeEffort(body.effort);
                    }
                    ...
                    if (typeof bodyProvider === 'string')
                        next.provider = bodyProvider.trim();
                    if (typeof bodyModel === 'string')
                        next.model = bodyModel.trim();
